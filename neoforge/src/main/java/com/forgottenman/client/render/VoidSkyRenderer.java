 ...
 * pack never consults -- it draws its own sky from its own programs, and a pack even
 * overrides the cloud setting. Forcing the fog colour black was tried and had no effect
 * either.
 *
...
public final class VoidSkyRenderer {
    // AFTER_LEVEL draw order is otherwise undefined: the void backdrop goes down first
    @SubscribeEvent(priority = EventPriority.HIGHEST)
    static void onRenderLevelStage(RenderLevelStageEvent event) {
        if (event.getStage() != RenderLevelStageEvent.Stage.AFTER_LEVEL
                || !ShaderCompat.useVanillaShaders() || ShaderCompat.isShadowPass()) {
            return;
        }
        ...
        builder.vertex(-1.0F, -1.0F, 1.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
        builder.vertex(1.0F, -1.0F, 1.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
        builder.vertex(1.0F, 1.0F, 1.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
        builder.vertex(-1.0F, 1.0F, 1.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();

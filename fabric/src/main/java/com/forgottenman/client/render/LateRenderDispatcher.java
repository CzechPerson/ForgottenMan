 ...
 *
 * Called from LevelRendererMixin at the tail of renderLevel -- the same point Forge
...
public final class LateRenderDispatcher {
    public static void render(float partialTick, Camera camera, Frustum frustum,
                              Matrix4f modelViewMatrix, Matrix4f projectionMatrix) {
        VoidSkyRenderer.render();
        InfiniteRoomRenderer.renderLate(partialTick, camera, frustum, modelViewMatrix, projectionMatrix);
        DoorPortalRenderer.renderLate(partialTick, camera, frustum, modelViewMatrix, projectionMatrix);
        LateManRenderer.render(camera, modelViewMatrix, partialTick);
